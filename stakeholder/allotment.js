    ...
    await db.getConnection(async (err, connection) => {
        ...
        await connection.query(searchquery, async (err, result) => {
            ...
            if (result.length != 0) {
                ...
                console.log(token)
                const insert = mysql.format('insert into faculty values(null,?,null,?)', [email, token])
                await connection.query(insert, async (err, result) => {
                    if (err) throw err;
                    const sub="Allotment- Research Nexas"
                    const link = `http://localhost:3000/fac_signup?token=${registrationToken(email)}`
                    ...
                    const update = mysql.format('update upload_file_db set fac_mail=? where sno=?', [email, paperid])
